...
const notificationReducer = (state, newNotificationData) => {
  ...
  switch (newNotificationData.type) {
    case "lowStock":
      formattedMessage = `Nízky stav zásob pre produkt ${
        newNotificationData.productName || "Neznámy produkt"
      }`;
      break;
    case "newOrder":
    ...
    case "orderUpdated":
      ...
      break;
    case "productCreated":
      formattedMessage = `Nový produkt bol vytvorený: ${
        newNotificationData.productName || "Neznámy produkt"
      }`;
      break;
    case "productUpdated":
      formattedMessage = `Produkt bol aktualizovaný: ${
        newNotificationData.productName || "Neznámy produkt"
      }`;
      break;
    case "userRegistered":
      formattedMessage = `Nový používateľ sa zaregistroval: ${
        newNotificationData.email || newNotificationData.userId
      }`;
      break;
    default:
...
export default function useAdminNotifications() {
  ...
  const handleAdminNotification = useCallback(
    (message) => {
      console.log(
        "useAdminNotifications: Received admin notification",
        message.data
      );
      if (message.data) {
