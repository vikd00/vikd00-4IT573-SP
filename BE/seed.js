...
async function main() {
  ...
  try {
    ...
    const adminPasswordHash = await bcrypt.hash("admin123", 10);    const testUser = await db.insert(schema.users).values({
      username: "test",
    ...
    const productResults = await db.insert(schema.products).values([
      {
    ...
    const testCart = await db.insert(schema.carts).values({
      userId: testUserId,
      createdAt: new Date().toISOString(),
    }).returning();

    const adminCart = await db.insert(schema.carts).values({
      userId: adminUserId,
      createdAt: new Date().toISOString(),
    ...
    const order4 = await db.insert(schema.orders).values({
      userId: testUserId,
      status: "pending",
      shippingAddress: "123 Main St, Cityville, State 12345, USA",
      createdAt: new Date().toISOString(),
